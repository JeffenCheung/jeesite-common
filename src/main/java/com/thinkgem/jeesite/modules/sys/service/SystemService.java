...
public class SystemService extends BaseService implements InitializingBean {
	...
	public void clearCacheOfAllUsers(){
		for (User e : findUser(new User())){
			UserUtils.clearCache(e);
		}
	}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	@Transactional(readOnly = false)
	public void updateCloseUser(User user) {
		user.preUpdate();
		userDao.updateCloseUser(user);
		// 清除用户缓存
		UserUtils.clearCache(user);
//		// 清除权限缓存
//		systemRealm.clearAllCachedAuthorizationInfo();
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void saveRole(Role role) {
		...
		if (Global.getConfigBoolean("widget.ztree.office.async")) {
			// add officeListByAsyc by jeffen@pactera 2015/12/31
			roleDao.deleteRoleOfficeListByAsyc(role);
		} else {
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void saveMenu(Menu menu) {
		...
		// add by jeffen@pactera 2015/10/8
		// 生成标准权限的下级隐藏菜单 (查看：view；编辑：edit；IO：io；DBA：dba)
		if(menu.isChecked()){
			// 获取[隐藏菜单]父节点实体
			Menu hideMenu = new Menu();
			hideMenu.setParent(menu);
			
			// 设置[隐藏菜单]新的父节点串
			hideMenu.setParentIds(menu.getParentIds()+menu.getId()+",");
			
			// 获取权限标识前缀
			String permissionPre = menu.getHref().substring(1, menu.getHref().length()).replaceAll("/", ":");
			// 清理末尾多余的冒号
			permissionPre = ":".equals(permissionPre.substring(permissionPre
					.length() - 1)) ? permissionPre.substring(0,
					permissionPre.length() - 1) : permissionPre;

			// 查询[隐藏菜单①-查看]实体
			hideMenu.setPermission(permissionPre+":view");
			List<Menu> viewMenuList = menuDao.findByPermission(hideMenu);
			
			// 保存[隐藏菜单①-查看]实体
			if (viewMenuList == null || viewMenuList.size()==0){
				hideMenu.preInsert();
				hideMenu.setName("查看");
				hideMenu.setSort(30);
				hideMenu.setIsShow("0");
				menuDao.insert(hideMenu);
			}

			// 查询[隐藏菜单②-编辑]实体
			hideMenu.setPermission(permissionPre+":edit");
			List<Menu> editMenuList = menuDao.findByPermission(hideMenu);
			
			// 保存[隐藏菜单②-编辑]实体
			if (editMenuList == null || editMenuList.size()==0){
				hideMenu.preInsert();
				hideMenu.setName("编辑");
				hideMenu.setSort(60);
				hideMenu.setIsShow("0");
			...
			// 查询[隐藏菜单③-IO]实体
			hideMenu.setPermission(permissionPre+":io");
			List<Menu> ioMenuList = menuDao.findByPermission(hideMenu);
			
			// 保存[隐藏菜单③-IO]实体
			if (ioMenuList == null || ioMenuList.size()==0){
				hideMenu.preInsert();
				hideMenu.setName("IO");
				hideMenu.setSort(90);
				hideMenu.setIsShow("0");
			...
			// 查询[隐藏菜单④-DBA]实体
			hideMenu.setPermission(permissionPre+":dba");
			List<Menu> dbaMenuList = menuDao.findByPermission(hideMenu);
			
			// 保存[隐藏菜单④-DBA]实体
			if (dbaMenuList == null || dbaMenuList.size()==0){
				hideMenu.preInsert();
				hideMenu.setName("DBA");
				hideMenu.setSort(120);
				hideMenu.setIsShow("0");
				menuDao.insert(hideMenu);
			}
		}
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public static boolean printKeyLoadMessage(){
		...
		sb.append("\r\n======================================================================\r\n");
		sb.append("\r\n    欢迎使用 "+Global.getConfig("productName")+"  - Powered By http://pactera.com\r\n");
		sb.append("\r\n======================================================================\r\n");
	...
	public List<User> getByOfficeCode(String code) {
		return userDao.getByOfficeCode(code);
	}

	public User getRtxById(String userId) {
		return userDao.getRtxById(userId);
	}
