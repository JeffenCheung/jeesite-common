...
(
	...
	login_flag varchar(64) COMMENT '是否可登录（0：否；1：是）',
	
	REOPEN_DATE datetime COMMENT '关停帐号截至时间',
	CLOSE_TYPE varchar(64) COMMENT '关停帐号原因类型',
	FLOOR_ID varchar(64) default '0' not null COMMENT '楼层ID',
	USER_LEVEL decimal(1,0) default 1 not null COMMENT '等级（1~9，数字越高代表等级越高）',
	SSID varchar(50) COMMENT 'AD域中对应的主键',
	ACCOUNT_TYPE decimal(1,0) default 0 not null COMMENT '账号类别：1-AD域导入；2-自建；',
	POST_ID varchar(64) default '0' not null COMMENT '职位ID',
...
(
	id varchar(64) NOT NULL COMMENT '编号',
	ORG_ID varchar(64) NOT NULL COMMENT '组织结构编码',
	POST_NAME varchar(200) DEFAULT 'NICK-POST' NOT NULL COMMENT '职位名称',
	POST_DES varchar(50) COMMENT '职位描述',
	POST_LEVEL decimal(1,0) DEFAULT '1' NOT NULL COMMENT '职位等级（1~9，数字越高代表等级越高）',
	create_by varchar(64) NOT NULL COMMENT '创建者',
	create_date datetime NOT NULL COMMENT '创建时间',
	update_by varchar(64) NOT NULL COMMENT '更新者',
	update_date datetime NOT NULL COMMENT '更新时间',
...
create index SYS_USER_POST_ID on SYS_USER (post_id ASC);
create index SYS_USER_NAME on SYS_USER (name ASC);

CREATE INDEX sys_post_ORG_ID ON sys_post (ORG_ID ASC);
CREATE INDEX sys_post_del_flag ON sys_post (del_flag ASC);
CREATE INDEX sys_post_POST_LEVEL ON sys_post (POST_LEVEL ASC);
