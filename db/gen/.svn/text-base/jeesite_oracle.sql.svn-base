...
(
	...
	sort number,
	IS_CSV CHAR(1),
	CSV_COLUMN_ACCESS_TYPE varchar2(50),
	CSV_POSITION NUMBER,
	DEFAULT_VALUE varchar2(500),
	FORMAT varchar2(500),
	IS_EXCEL CHAR(1),
	EXCEL_VALUE varchar2(500),
	EXCEL_TITLE varchar2(500),
	EXCEL_TYPE varchar2(2),
	EXCEL_ALIGN  varchar2(2),
...
COMMENT ON COLUMN gen_scheme.del_flag IS '删除标记（0：正常；1：删除；2：审核）';
COMMENT ON COLUMN gen_scheme.IO_ARTIFACT IS '导入导出工具包';
COMMENT ON COLUMN gen_scheme.PROJECT_PATH IS '工程路径';
...
COMMENT ON COLUMN gen_table.DATA_SOURCE IS '数据源（空：默认；2：数据源2；3：数据源3；4：数据源4；5：数据源5；n：数据源n）';
...
COMMENT ON COLUMN gen_table.del_flag IS '删除标记（0：正常；1：删除；2：审核）';
...
COMMENT ON COLUMN gen_table_column.del_flag IS '删除标记（0：正常；1：删除；2：审核）';
COMMENT ON COLUMN gen_table_column.IS_CSV IS '是否CSV字段';
COMMENT ON COLUMN gen_table_column.CSV_COLUMN_ACCESS_TYPE IS 'CSV列数据访问方式：[READ]导入只读，但不导该出字段；[READ_WRITE]导入只读，并导该出字段；[WRITE]导入文件不含该字段，但导该出字段';
COMMENT ON COLUMN gen_table_column.CSV_POSITION IS 'CSV文件列位置';
COMMENT ON COLUMN gen_table_column.DEFAULT_VALUE IS '默认值';
COMMENT ON COLUMN gen_table_column.FORMAT IS '格式化(#,##0.0;yyyy/MM/dd;HH:mm:ss)';
COMMENT ON COLUMN gen_table_column.IS_EXCEL IS '是否EXCEL字段';
COMMENT ON COLUMN gen_table_column.EXCEL_VALUE IS 'EXCEL导出字段名（默认调用当前字段的“get”方法，如指定导出字段为对象，请填写“对象名.对象属性”，例：“area.name”、“office.name”）';
COMMENT ON COLUMN gen_table_column.EXCEL_TITLE IS 'EXCEL导出字段标题（需要添加批注请用“**”分隔，标题**批注，仅对导出模板有效）';
COMMENT ON COLUMN gen_table_column.EXCEL_TYPE IS 'EXCEL字段类型（0：导出导入；1：仅导出；2：仅导入）';
COMMENT ON COLUMN gen_table_column.EXCEL_ALIGN IS 'EXCEL导出字段对齐方式（0：自动；1：靠左；2：居中；3：靠右）';
COMMENT ON COLUMN gen_table_column.ORDER_TYPE IS '结果排序方式（非排序字段、升序ASC、降序DESC）';
...
COMMENT ON COLUMN gen_template.del_flag IS '删除标记（0：正常；1：删除；2：审核）';
